...
def tokenize_structure(  # noqa: C901, PLR0915
    ...
    interface_chain_id: Optional[int] = None,
) -> tuple[np.ndarray, np.ndarray]:
    ...
    for chain in chains:
        ...
        interface_mask = (interface_chain_id is not None) and (
           int(chain["asym_id"]) == int(interface_chain_id)
        )
...
class Boltz2Tokenizer(Tokenizer):
    ...
    def __init__(self, interface_chain_id: Optional[int] = None):
       super().__init__()
       self.interface_chain_id = interface_chain_id
    ...
    def tokenize(self, data: Input) -> Tokenized:
        ...
        token_data, token_bonds = tokenize_structure(
            data.structure, data.record.affinity, interface_chain_id=self.interface_chain_id,
        )
