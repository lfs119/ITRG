...
def resolve_interface_chain_id(
    record: Record,
    interface_chain_name: Optional[str],
) -> Optional[int]:
    """Map user-facing chain name to asym_id (stored as chain_id in Record)."""
    if interface_chain_name is None:
        return None

    for chain in record.chains:
       if str(chain.chain_name) == str(interface_chain_name):
            return int(chain.chain_id)

    available = [str(chain.chain_name) for chain in record.chains]
    msg = (
        f"Could not find interface chain '{interface_chain_name}' in record {record.id}. "
...
class PredictionDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        affinity: bool = False,
        interface_chain_name: Optional[str] = None,
    ) -> None:
        ...
        self.interface_chain_name = interface_chain_name
        if self.affinity:
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Tokenize structure
        try:
            interface_chain_id = resolve_interface_chain_id(
               record=record,
               interface_chain_name=self.interface_chain_name,
           )
            tokenizer = Boltz2Tokenizer(interface_chain_id=interface_chain_id)
        ...
        try:
            ...
            # asym_id generate interface_token_mask   support  protein/RNA/DNA/peptide 
            if interface_chain_id is not None:
                features["interface_token_mask"] = (
                    (features["asym_id"] == int(interface_chain_id)).float()
                    * features["token_pad_mask"].float()
                )
            else:
                features["interface_token_mask"] = torch.zeros_like(
                    features["token_pad_mask"], dtype=torch.float
                )

            features["partner_token_mask"] = features["interface_token_mask"]
        except Exception as e:  # noqa: BLE001
            import traceback
