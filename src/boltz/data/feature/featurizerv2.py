...
def process_token_features(  # noqa: C901, PLR0915, PLR0912
    ...
    if "interface_mask" in token_data.dtype.names:
           interface_mask = from_numpy(token_data["interface_mask"]).float()
    else:
        interface_mask = torch.zeros(len(token_data), dtype=torch.float)
    ...
    if max_tokens is not None:
        ...
        if pad_len > 0:
            ...
            interface_mask = pad_dim(interface_mask, 0, pad_len)
    ...
    token_features = {
        ...
        "interface_token_mask": interface_mask,
    }
