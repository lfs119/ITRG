...
class Boltz2(LightningModule):
                ...
                if self.checkpoint_diffusion_conditioning and self.training:
                   ...
                   forward_out = (
                        torch.utils.checkpoint.checkpoint(
                            ...
                            feats,
                            steering_args=self.steering_args,   # NEW
                        )
                    )
                else:
                    forward_out = (
                        self.diffusion_conditioning(
                            ...
                            feats=feats,
                            steering_args=self.steering_args,   # add interface steering NEW
                        )
                    )
                     
                use_steering = (
                    self.steering_args is not None
                    and self.steering_args.get("enable_interface_steering", False)
                )
                
                feats["ligand_atom_index"] = self.steering_args.get("partner_atom_indices", None)
                feats["interface_protein_atom_indices"] = self.steering_args.get("interface_protein_atom_indices", None)
                feats["interface_protein_atom_types"] = self.steering_args.get("interface_protein_atom_types", None)
                feats["interface_partner_atom_indices"] = self.steering_args.get("interface_partner_atom_indices", None)
                feats["interface_protein_atom_type_onehot"] = self.steering_args.get("interface_protein_atom_type_onehot", None)
                feats["interface_ligand_atom_type_onehot"] = self.steering_args.get("interface_partner_atom_type_onehot", None)
                feats["atom_type_vocab"] = self.steering_args.get("atom_type_vocab", None)
                feats["interface_contact_pair_index"] = self.steering_args.get("interface_contact_pair_index", None)  # interface_contact_pair_index
                feats["protein_atom_indices"] = self.steering_args.get("protein_atom_indices", None)
                feats["protein_atom_type_onehot"] = self.steering_args.get("protein_atom_type_onehot", None)
                feats["partner_atom_type_onehot"] = self.steering_args.get("partner_atom_type_onehot", None)
                

                if not use_steering:
                    q, c, to_keys, atom_enc_bias, atom_dec_bias, token_trans_bias = forward_out
                    ...
                    # ---- steering / MCMC ----
                    sample_outputs, z_final, accept_rate = forward_out

                    print(f"[downstream] interface steering accept_rate = {accept_rate.mean().item():.4f}")

                    struct_out_list = []

                    for sample_out in sample_outputs:
                        diffusion_conditioning = {
                            "q": sample_out["q"],
                            "c": sample_out["c"],
                            "to_keys": sample_out["to_keys"],
                            "atom_enc_bias": sample_out["atom_enc_bias"],
                            "atom_dec_bias": sample_out["atom_dec_bias"],
                            "token_trans_bias": sample_out["token_trans_bias"],
                        }
                        

                        with torch.autocast("cuda", enabled=False):
                            struct_out_i = self.structure_module.sample(
                                s_trunk=s.float(),
                                s_inputs=s_inputs.float(),
                                feats=feats,
                                num_sampling_steps=num_sampling_steps,
                        ...
                        struct_out_i["sample_idx"] = sample_out["sample_idx"]
                        struct_out_i["z_unsteered"] = dict_out["z"]

                        struct_out_list.append(struct_out_i)

                    dict_out["struct_out_list"] = struct_out_list
                    dict_out["z_final"] = z_final
                    dict_out["mcmc_accept_rate"] = accept_rate
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> dict:
        try:
            ...
            if "struct_out_list" in out:
                pred_dict["z"] = out.get("z_final", None)

                struct_out_list = out["struct_out_list"]
                coords_list = [x["sample_atom_coords"] for x in struct_out_list]

                try:
                    coords_list_stack = torch.stack(coords_list, dim=0)
                    pred_dict["coords"] = coords_list_stack.reshape(-1, *coords_list_stack.shape[2:])
                    
                except Exception:
                    pred_dict["coords"] = coords_list

                pred_dict["coords_list"] = coords_list
                pred_dict["mcmc_accept_rate"] = out.get("mcmc_accept_rate", None)

                if len(coords_list) > 0:
                    pred_dict["coords_first"] = coords_list[0]
