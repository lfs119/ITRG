...
from boltz.pocket_patch_utils import build_protein_patch_mask_for_boltz2, extract_interface_patch_and_index_from_prior_complex, extract_interface_patch_from_prior_complex
...
class BoltzSteeringParams:
    ...
    # ---- NEW: interface prior steering (z_trunk -> diffusion_conditioning) ----
    enable_interface_steering: bool = False
    
    enable_protein_patch_mask: bool = False
    protein_patch_mask: Optional[torch.Tensor] = None  # if True, use provided mask to steer only interface residues; else steer whole protein
    max_mcmc_samples: int = 5
    ligand_diversity_mode: str = "conformer"
    partner_atom_indices: Optional[torch.Tensor] = None
    protein_atom_indices: Optional[torch.Tensor] = None
    interface_protein_atom_indices: Optional[torch.Tensor] = None
    interface_partner_atom_indices: Optional[torch.Tensor] = None

    partner_mask_mode: str = "auto"
    
    prior_complex_path: Optional[str] = None  # if provided, use this known complex to extract interface patch for steering; overrides enable_protein_patch_mask
    # steer z AFTER pairwise_conditioner (recommended)
    beta_z_interface: float = -0.3          # masked scaling of z on interface pairs
    include_partner_partner: bool = False   # include partner<->partner block in pair_mask
    exclude_partner_diag: bool = True       # exclude partner-partner diagonal

    # optional masked additive update on z (directional steering)
    gamma_z_interface: float = 0.0
    z_add_vector: Optional[Sequence[float]] = None  # length token_z; None disables

    # steer token-level attention bias directly (stable)
    lambda_token_bias: float = 0.0          # add lam * pair_mask to token_trans_bias

    # (optional) separate strengths for rec<->partner and partner<->partner
    beta_z_pp: float = 0.0                  # if include_partner_partner=True, extra scaling on pp block
    lambda_token_bias_pp: float = 0.0       # extra bias on pp block

    # (optional) debug exports / logging
    log_interface_mask_stats: bool = True
    
    protein_atom_types: Optional[torch.Tensor] = None
    partner_atom_types: Optional[torch.Tensor] = None
    interface_protein_atom_types: Optional[torch.Tensor] = None
    interface_partner_atom_types: Optional[torch.Tensor] = None
    protein_atom_type_onehot: Optional[torch.Tensor] = None
    partner_atom_type_onehot : Optional[torch.Tensor] = None
    interface_protein_atom_type_onehot: Optional[torch.Tensor] = None
    interface_partner_atom_type_onehot: Optional[torch.Tensor] = None
    atom_type_vocab: Optional[torch.Tensor] = None
    interface_contact_pair_index: Optional[torch.Tensor] = None
...
@click.option(
    ...
    type=str,
    default=None,
    help="Optional run flag used to name the result directory.",
)
@click.option(
    "--cache",
    type=click.Path(exists=False),
...
@click.option(
    ...
    is_flag=True,
    help=" to enable interface steering. Default is False.",
)

@click.option(
    "--enable_protein_patch_mask",
    is_flag=True,
    help=" to enable protein patch mask. Default is False.",
)   
@click.option(
    "--prior_complex_path",
    type=str,
    help="The path to the prior complex pdb file.",
    default=None,
)
# beta_z_interface
@click.option(
    "--beta_z_interface",
    type=float,
    help="The beta value for interface steering. Default is -0.3.",
    default=-0.3
)   
@click.option(
    "--max_mcmc_samples",
    type=int,
    help="The maximum number of MCMC samples to use for prediction. Default is 5.",
    default=5,
)

@click.option("--ligand_diversity_mode",
    type=click.Choice(["None", "pose",  "conformer", "environment"]),
    help="The ligand diversity mode to use for prediction. Default is conformer.",
    default="None",
)
@click.option(
    "--protein_chain_names",
    multiple=True,
    help="The chain names of the protein chains to use for prediction. Default is all.",
    default=["A"],
)
@click.option( 
    "--partner_chain_names",
    multiple=True,
    help="The chain names of the parent chains to use for prediction. Default is all.",
    default=["B"],
)
@click.option(
    "--interface_chain",
    type=str,
    help="The name of the interface chain to use for prediction. Default is B.",
    default="B",
)
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    enable_interface_steering: bool = False,
    beta_z_interface: float = 0.3,
    max_mcmc_samples: int = 5,
    enable_protein_patch_mask: bool = False,
    prior_complex_path: Optional[str] = None,
    ligand_diversity_mode: Optional[str] = None,
    protein_chain_names: List[str] = ["A"],
    partner_chain_names: List[str] = ["B"],
    ...
    run_name = flag if flag else data.stem
    out_dir = out_dir / f"boltz_results_{run_name}_{data.stem}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if filtered_manifest.records:
        ...
        if model == "boltz2":
            data_module = Boltz2InferenceDataModule(
                ...
                interface_chain_name=interface_chain,
            )
        else:
        ...
        steering_args.enable_interface_steering = enable_interface_steering
        steering_args.beta_z_interface = beta_z_interface
        steering_args.enable_protein_patch_mask = enable_protein_patch_mask
        steering_args.max_mcmc_samples = max_mcmc_samples
        steering_args.prior_complex_path = prior_complex_path
        steering_args.ligand_diversity_mode = ligand_diversity_mode
        if enable_protein_patch_mask and steering_args.prior_complex_path is not None:
            # (
            #     interface_inf,
            #     protein_atom_indices,
            #     partner_atom_indices,
            #     interface_protein_atom_indices,
            #     interface_partner_atom_indices
            # ) = extract_interface_patch_and_index_from_prior_complex(steering_args.prior_complex_path, protein_chain_names=["A"], partner_chain_names=["B"])
            ...
            atom_type_vocab,
            ) = extract_interface_patch_and_index_from_prior_complex(
            cif_path=steering_args.prior_complex_path,
            protein_chain_names=protein_chain_names,
            partner_chain_names=partner_chain_names,
            cutoff=4.5,)
            
            steering_args.partner_atom_indices = partner_atom_indices
            steering_args.protein_atom_indices = protein_atom_indices
            steering_args.interface_protein_atom_indices = interface_protein_atom_indices
            steering_args.interface_partner_atom_indices = interface_partner_atom_indices
            steering_args.protein_atom_type_onehot = protein_atom_type_onehot
            steering_args.partner_atom_type_onehot = partner_atom_type_onehot
            steering_args.interface_protein_atom_type_onehot = interface_protein_atom_type_onehot
            steering_args.interface_partner_atom_type_onehot = interface_partner_atom_type_onehot
            steering_args.protein_atom_types = protein_atom_types
            steering_args.partner_atom_types = partner_atom_types
            steering_args.interface_protein_atom_types = interface_protein_atom_types
            steering_args.interface_partner_atom_types = interface_partner_atom_types
            steering_args.atom_type_vocab = atom_type_vocab
            steering_args.interface_contact_pair_index = interface_contact_pair_index
            
            protein_patch_mask = build_protein_patch_mask_for_boltz2(
                processed.manifest.records[0],
                processed.targets_dir,
                interface_inf,
            )
            steering_args.protein_patch_mask = protein_patch_mask.cuda() if torch.cuda.is_available() else protein_patch_mask
            # print(protein_patch_mask.shape, protein_patch_mask.sum())
